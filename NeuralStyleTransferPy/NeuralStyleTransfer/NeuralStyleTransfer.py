...
#source to the content image and the style image to apply to generate image. Images must be 400w x 300h
contentImagePath = "images/louvre_small.jpg"
styleImagePath = "images/monet.jpg"

content_image = plt.imread(contentImagePath)
...
def compute_content_cost(a_C, a_G):
    ...
    J_content = tf.reduce_sum(input_tensor=tf.square(tf.subtract(a_C_unrolled,a_G_unrolled))) / (4 * n_H * n_W * n_C)
    ### END CODE HERE ###
...
style_image = plt.imread(styleImagePath)
imshow(style_image)
...
def compute_layer_style_cost(a_S, a_G):
    ...
    GS = gram_matrix(tf.transpose(a=a_S))
    GG = gram_matrix(tf.transpose(a=a_G))

    # Computing the loss (≈1 line)
    J_style_layer = tf.reduce_sum(input_tensor=tf.square(tf.subtract(GS,GG))) / (4 * (n_C * n_C) * (n_W * n_H) * (n_W * n_H))
...
filename = os.path.basename(contentImagePath)
(file, ext) = os.path.splitext(filename)
contentFileName = file.replace("-400x300", "")

filename = os.path.basename(styleImagePath)
(file, ext) = os.path.splitext(filename)
styleFileName = file.replace("-400x300", "")

os.mkdir("output/" + contentFileName + "-" + styleFileName + "-lr-" + str(learning_rate) + "/")
